...
(function ($) {
	...
	var constants = {
		iRectSize: 8,
		symbolSize: 8,
		mouseGrabMargin: 20,	// margin on each side of cursor line; hitbox width = mouseGrabMargin*2
		anchorMargin: 40,		// additional margin to extend length of cursor's hitbox by
	...
	function init(plot) {
		...
		function createCursor(options) {
			return mixin(options, {
				...
				mouseCursors: {},
				dashes: 1,
				intersectionColor: options.color || 'darkgray',
				intersectionLabelPosition: 'bottom-right'
			});
		}
		var mouseCursors ={
			default: 'default',
			mouseOverCursorVerticalLine: 'col-resize',
			mouseOverCursorHorizontalLine: 'row-resize',
			mouseOverCursorManipulator: 'pointer',
			panCursor: 'move',
		};
		plot.mouseCursors = mouseCursors;
		...
		plot.addCursor = function addCursor(options) {
			...
			$.extend(plot.mouseCursors, options.mouseCursors);
		...
		function onTouchDown(e) {
			...
			if (currentlySelectedCursor) {
				...
				plot.getPlaceholder().children().css('cursor', mouseCursors.default);
				...
				var targetCursor;
				var dragmode;
				var minMouseDistance = Infinity;

				visibleCursors(cursors).forEach(function (cursor) {
					if (!cursor.movable) {
						return;
					}
					if (mouseOverCursorHorizontalLine(e, plot, cursor) && cursor.mouseDistance < minMouseDistance && (cursor.mode == 'y' || cursor.snapToPlot === undefined)) {
						targetCursor = cursor;
						dragmode = 'y';
						minMouseDistance = cursor.mouseDistance;
					}
					if (mouseOverCursorVerticalLine(e, plot, cursor) && cursor.mouseDistance < minMouseDistance) {
						targetCursor = cursor;
						dragmode = 'x';
						minMouseDistance = cursor.mouseDistance;
					}
				...
				if (targetCursor) {
					...
					if (targetCursor.mode === 'x') {
						plot.getPlaceholder().css('cursor', mouseCursors.mouse);
					...
					*/
					plot.getPlaceholder().children().css('cursor', mouseCursors.panCursor);
					plot.triggerRedrawOverlay();
					plot.getPlaceholder().trigger("cursorSelected");
					e.stopPropagation();
				}
			}
		}
		...
		function onTouchUp(e) {
			var offset = plot.offset();
            var mouseX = Math.max(0, Math.min(e.originalEvent.changedTouches[0].pageX - offset.left, plot.width()));
            var mouseY = Math.max(0, Math.min(e.originalEvent.changedTouches[0].pageY - offset.top, plot.height()));
			var currentlySelectedCursor = selectedCursor(cursors);
			...
			if (currentlySelectedCursor) {
				...
				visibleCursors(cursors).forEach(function (cursor) {
					...
					if (mouseOverCursorManipulator(e, plot, cursor)) {
						...
						plot.getPlaceholder().children().css('cursor', mouseCursors.mouseOverCursorManipulator);
					}
					else if (mouseOverCursorVerticalLine(e, plot, cursor) && cursor.mouseDistance < minMouseDistance) {
						if (!cursor.highlighted) {
							cursor.highlighted = true;
							plot.triggerRedrawOverlay();
						}
						minMouseDistance = cursor.mouseDistance;
						plot.getPlaceholder().children().css('cursor', mouseCursors.mouseOverCursorVerticalLine);
					}
					else if (mouseOverCursorHorizontalLine(e, plot, cursor) && cursor.mouseDistance < minMouseDistance && (cursor.mode == 'y' || cursor.snapToPlot === undefined)) {
						if (!cursor.highlighted) {
							cursor.highlighted = true;
							plot.triggerRedrawOverlay();
						}
						minMouseDistance = cursor.mouseDistance;
						plot.getPlaceholder().children().css('cursor', mouseCursors.mouseOverCursorHorizontalLine);
					} else {
						if (cursor.highlighted) {
		...
		plot.hooks.bindEvents.push(function (plot, eventHolder) {
			...
			plot.getPlaceholder().bind('mousemove', onMouseMove);
			plot.getPlaceholder().bind('mousedown', onMouseDown);
			plot.getPlaceholder().bind('mouseup', onMouseUp);
			plot.getPlaceholder().bind('mouseleave', onMouseOut);

			//Touch Events
			eventHolder.bind('touchstart', onTouchDown);
			eventHolder.bind('touchmove', onTouchMove);
			eventHolder.bind('touchend', onTouchUp);

			plot.getPlaceholder().bind('touchmove', onTouchMove);
			plot.getPlaceholder().bind('touchstart', onTouchDown);
			plot.getPlaceholder().bind('touchend', onTouchUp);
			plot.getPlaceholder().bind('touchleave', onTouchOut);
		});

		// For a given x, finds the corresponding y coordinate for the intersection with EACH data series
		// Currently, does not support tracking x values, based on y
		function findIntersections(plot, cursor) {
			...
			for (i = 0; i < dataset.length; ++i) {
				...
				var xaxis = "x" + series.xaxis.n;
				var yaxis = "y" + series.yaxis.n;

				if (pos[xaxis] < series.xaxis.min || pos[xaxis] > series.xaxis.max ||
					pos[yaxis] < series.yaxis.min || pos[yaxis] > series.yaxis.max) {
					continue;
				}

				// Find the nearest points, x-wise
				for (j = 0; j < series.data.length; ++j) {
					if (series.data[j] && series.data[j][0] > pos[xaxis]) {
						break;
					}
				}
				...
				// For scaled data, flot linearly interpolates data AFTER scaling the data points
				// So, we need to interpolate not the original data, but the scaled data
				var id = function(i) { return i; };
				var yTrans = series.yaxis.options.transform || id;
				var yInv = series.yaxis.options.inverseTransform || id;
				var xTrans = series.xaxis.options.transform || id;

				var slope = (yTrans(p2[1]) - yTrans(p1[1])) / (xTrans(p2[0]) - xTrans(p1[0]));
				var intercept = yTrans(p1[1]);
				var xDist = xTrans(pos[xaxis]) - xTrans(p1[0]);
				y = yInv(intercept + (slope * xDist));

				// Extreme Edge Case: When flot interpolates (connects data points with straight lines),
				// it does not consider data points that lie outside each axis' min/max values.
				// Instead, it clips the plot by adding artificial data points at the edges.
				// So with very few data points, our interpolation will differ from flot's
				// and the cursors may not display the intersections correctly.
		...
		plot.hooks.shutdown.push(function (plot, eventHolder) {
			...
			plot.getPlaceholder().unbind('mousemove', onMouseMove);
			plot.getPlaceholder().unbind('mousedown', onMouseDown);
			plot.getPlaceholder().unbind('mouseup', onMouseUp);
			plot.getPlaceholder().unbind('mouseleave', onMouseOut);

			//Touch Events
			eventHolder.unbind('touchstart', onTouchDown);
			eventHolder.unbind('touchmove', onTouchMove);
			eventHolder.unbind('touchend', onTouchUp);
			plot.getPlaceholder().unbind('touchleave', onTouchOut);
			plot.getPlaceholder().unbind('touchmove', onTouchMove);
			plot.getPlaceholder().unbind('touchstart', onTouchDown);
			plot.getPlaceholder().unbind('touchend', onTouchUp);
	...
	function setPosition(plot, cursor, pos) {
		...
		var series = plot.getData()[cursor.snapToPlot || 0];
		if (series){
			o = {
				top: series.yaxis.p2c(pos.y),
				left: series.xaxis.p2c(pos.x),
			};
		}
		else{
		o = plot.p2c(pos);
	...
	function maybeSnapToPlot(plot, cursor, intersections) {
		if (cursor.mode != 'y' && cursor.snapToPlot !== undefined) {
	 ...
	 * The bottom one has the index = 0, and the top one has the index = count -1. Depending on the current cursor's position
	...
	function fillTextAligned(ctx, text, x, y, position, plotWidth, plotHeight, fontStyle, fontWeight, fontSize, fontFamily) {
		var fontSizeInPx = Number(fontSize.substring(0, fontSize.length - 2));
		var textX = x, textY = y;
		var textWidth = ctx.measureText(text).width;
		switch (position) {
			case 'left':
				textX = x - textWidth - constants.iRectSize;
				textY = y;
				break;
			case 'bottom-left':
				textX = x - textWidth - constants.iRectSize;
				textY = y + fontSizeInPx;
				break;
			case 'top-left':
				textX = x - textWidth - constants.iRectSize;
				textY = y - constants.iRectSize;
				break;
			case 'top-right':
				textX = x + constants.iRectSize;
				textY = y - constants.iRectSize;
				if (textX + textWidth > plotWidth){
					// text will overflow canvas, draw on top-left instead
					textX = x - textWidth - constants.iRectSize;
				}
				break;
			case 'right':
				...
				textY = y;
				if (textX + textWidth > plotWidth){
					// text will overflow canvas, draw on left instead
					textX = x - textWidth - constants.iRectSize;
				}
				break;
			case 'bottom-right':
			default:
				...
				textY = y + fontSizeInPx;
				if (textX + textWidth > plotWidth){
					// text will overflow canvas, draw on bottom-left instead
					textX = x - textWidth - constants.iRectSize;
				}
				break;
		}
		if (textX < 0 || textX > plotWidth || textY < 0 || textY > plotHeight){
			return;
		}
	...
	function drawIntersections(plot, ctx, cursor) {
		if (cursor.showIntersections && hasVerticalLine(cursor)) {
			...
			ctx.beginPath();
			ctx.strokeStyle = $.color.parse(cursor.intersectionColor).scale('a', 0.5).toString();
			ctx.lineWidth = 3;
			var radius = 6;
			cursor.intersections.points.forEach(function (point, index) {
				...
				var coord = {
					left: point.dataSeries.xaxis.p2c(point.x),
					top: point.dataSeries.yaxis.p2c(point.y),
				};
				var plotWidth = plot.width(), plotHeight = plot.height();

				if (coord.left < 0 || coord.left > plotWidth || coord.top < 0 || coord.top > plotHeight){
					return;
				}
				ctx.moveTo(coord.left - radius, coord.top);
				ctx.lineTo(coord.left, coord.top - radius);
				ctx.lineTo(coord.left + radius, coord.top);
				ctx.lineTo(coord.left, coord.top + radius);
				ctx.closePath();
				...
				fillTextAligned(ctx, text, coord.left, coord.top, cursor.intersectionLabelPosition, plotWidth, plotHeight, cursor.fontStyle, cursor.fontWeight, cursor.fontSize, cursor.fontFamily);
			});
			ctx.stroke();
		}
	}
	...
	function mouseOverCursorManipulator(e, plot, cursor) {
		var offset = plot.offset();
        var pageX = getPositionOnPage(e)[0];
        var pageY = getPositionOnPage(e)[1];
	...
	function mouseOverCursorVerticalLine(e, plot, cursor) {
        ...
        var mouseX = pageX - offset.left;
        var mouseY = pageY - offset.top;
		var mouseGrabMarginY = cursor.drawAnchor ? (-1)*constants.anchorMargin : 0;

		cursor.mouseDistance = Math.abs(mouseX - cursor.x);
	...
	function mouseOverCursorHorizontalLine(e, plot, cursor) {
        ...
        var mouseX = pageX - offset.left;
        var mouseY = pageY - offset.top;
		var mouseGrabMarginX = cursor.drawAnchor ? (-1)*constants.anchorMargin : 0;

		cursor.mouseDistance = Math.abs(mouseY - cursor.y);
